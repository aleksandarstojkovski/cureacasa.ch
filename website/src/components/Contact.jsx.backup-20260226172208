...
const Contact = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!serviceId || !templateId || !publicKey) {
            setStatus({
                type: 'error',
                message: 'Configurazione EmailJS mancante. Controlla il file .env'
            });
            return;
        }
        ...
        try {
            ...
            setStatus({
                type: 'success',
                message: 'Messaggio inviato con successo! Ti contatterò al più presto.'
            });
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="flex items-start gap-4 p-4 bg-slate-50 rounded-lg">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-600">Locarno e dintorni, Svizzera</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-8 rounded-2xl shadow-sm border border-slate-100">
                        ...
                        <form ref={formRef} className="space-y-4" onSubmit={handleSubmit}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label htmlFor="user_name" className="block text-sm font-medium text-slate-700 mb-1">Nome</label>
                                    <input
                                        type="text"
                                        id="user_name"
                                        name="user_name"
                                ...
                                <div>
                                    <label htmlFor="user_phone" className="block text-sm font-medium text-slate-700 mb-1">Telefono</label>
                                    <input
                                        type="tel"
                                        id="user_phone"
                                        name="user_phone"
                            ...
                            {status.message && (
                                <div className={`p-4 rounded-lg flex items-center gap-3 ${status.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' :
                            ...
                            <button
                                ...
                                className={`w-full btn bg-primary-600 text-white py-3 rounded-lg font-semibold hover:bg-primary-700 transition-colors flex items-center justify-center gap-2 ${status.type === 'loading' ? 'opacity-70 cursor-not-allowed' : ''}`}
                            >
                                {status.type === 'loading' ? 'Invio...' : 'Invia Messaggio'}
                                {status.type !== 'loading' && <Send className="h-4 w-4" />}
                            </button>
                        </form>
                    </div>
                </div>
